...
local function WarnTime(cid, time, challengeUniqueId)
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	for _, p in pairs(playersTable) do
		...
		if nPlayer then
			...
			nPlayer:setStorageValue(ChallangeStorages.duelAreaUid, item.uid)
			nPlayer:setStorageValue(ChallangeStorages.duelSubAreaUid, asreaIndex)
		...
		nPlayer:sendTextMessage(MESSAGE_STATUS_WARNING, "The challenge has been start! Kill all of them! You have ".. math.floor(config.timeLapse / 1000 / 60).." minutes or this will end in a draw.")
	end
